...
(function() {
  ...
  this["new"] = function(settings) {
    var R, isl_settings, ref, ref1, substrate;
    isl_settings = {
      minIndex: σ_minus_א,
      maxIndex: σ_plus_א,
    ...
    R = {
      ...
      'indexes': (ref1 = settings != null ? settings['indexes'] : void 0) != null ? ref1 : {}
    };
    return R;
  };

  this.copy = function(me) {
    var R, entry, i, len, name, ref, settings;
    settings = {
  ...
  this.match = function(me, points, settings) {
    ...
    return this._cover_or_intersect(me, 'match', points, settings);
  };
  ...
  this.intersect = function(me, points, settings) {
    ...
    return this._cover_or_intersect(me, 'intersect', points, settings);
  };

  this._cover_or_intersect = function(me, mode, points, settings) {
  ...
  this._find_ids_with_any_points = function(me, points) {
    var R, i, id, ids, j, len, len1, point, ref;
  ...
  this.aggregate = function(me, point, reducers) {
    if (reducers == null) {
      reducers = null;
    }
    return mix = this.aggregate._get_mix(reducers);
  };

  this.aggregate._get_mix = (function(_this) {
    return function(reducers) {
      var id;
      if ((reducers == null) || (Object.keys(reducers)).length === 0) {
        return _this.aggregate.mix;
      }
      id = CND.id_from_text(CND.XJSON.stringify(reducers), 12);
      return _this.aggregate._cached_mixes[id] = mix.use(reducers);
    };
  })(this);

  this.aggregate._cached_mixes = {};

  this.aggregate.reducers = {
    idx: 'skip',
    id: 'skip',
    lo: 'skip',
    hi: 'skip',
  ...
  this.aggregate.mix = mix.use(this.aggregate.reducers);

  this._OLD_aggregate = function(me, point, reducers) {
    var R, averages, cache, count, entries, entry, exclude, functions, i, id, ids, ids_and_values, key, len, point_count, reducer, reducer_fallback, reducer_include, ref, ref1, ref2, ref3, ref4, sum, tag_keys, target, value, values;
    if (reducers == null) {
      reducers = {};
    }
    ...
    cache = {};
    averages = {};
    reducers = Object.assign({}, reducers, (ref = me['reducers']) != null ? ref : {});
    tag_keys = (function() {
      var results;
      results = [];
      for (key in reducers) {
    ...
    if (reducer_include === 'skip') {
      reducer_include = 'assign';
    }
    functions = {};
    for (key in reducers) {
    ...
    for (i = 0, len = entries.length; i < len; i++) {
      ...
      for (key in entry) {
        ...
        reducer = (ref2 = reducers[key]) != null ? ref2 : reducer_fallback;
        switch (reducer) {
          case 'skip':
            continue;
          case 'list':
    ...
    for (key in cache) {
      ...
      ids = (function() {
        ...
        results = [];
        for (j = 0, len1 = ids_and_values.length; j < len1; j++) {
          ref5 = ids_and_values[j], id = ref5[0], value = ref5[1];
          results.push(id);
        }
        return results;
      })();
