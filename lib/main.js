...
(function() {
  var CND, alert, append, as_number, as_numbers, badge, debug, echo, fuse, help, info, log, meld, minus_aleph, normalize_points, normalize_tag, plus_aleph, reduce_tag, rpr, setting_keys_of_cover_and_intersect, sort_entries_by_insertion_order, sort_ids_by_insertion_order, unique, urge, warn, whisper,
    slice = [].slice,
  ...
  this.match_common = function(me, points, settings) {
    var R, arity, i, len, pick, point, s;
    ...
    pick = settings.pick;
    if (pick == null) {
      throw new Error("expected setting 'pick', got none");
    }
    points = normalize_points(points);
    if (points.length === 0) {
      return [];
    ...
    for (i = 0, len = points.length; i < len; i++) {
      point = points[i];
      append(R, this.match(me, point, s));
    }
    if (pick === 'tag') {
      return reduce_tag(R);
    }
