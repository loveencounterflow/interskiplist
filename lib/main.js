...
(function() {
  var CND, alert, append, as_number, as_numbers, badge, debug, deepcopy_entry, deepcopy_reducers, echo, fuse, help, info, log, meld, minus_aleph, normalize_points, normalize_tag, plus_aleph, reduce_tag, rpr, setting_keys_of_cover_and_intersect, sort_entries_by_insertion_order, sort_ids_by_insertion_order, unique, urge, warn, whisper,
    slice = [].slice,
  ...
  this.copy = function(me) {
    var R, entry, i, len, ref, settings;
    settings = {
      reducers: deepcopy_reducers(me)
    };
    R = this["new"](settings);
    ref = this.entries_of(me);
    for (i = 0, len = ref.length; i < len; i++) {
      entry = ref[i];
      this.add(R, deepcopy_entry(null, entry));
    }
    return R;
  };
  ...
  this.add_index = function(me, name) {
    var indexes;
    indexes = me['indexes'] != null ? me['indexes'] : me['indexes'] = {};
    indexes[name] = {};
    return null;
  };
  ...
  this._index_entry = function(me, entry) {
    var i, idx, index, indexes, len, name, ref, tag, value;
    idx = entry.idx;
    if ((indexes = me['indexes']) != null) {
      for (name in entry) {
        ...
        if (name === 'tag') {
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            tag = ref[i];
            (index[tag] != null ? index[tag] : index[tag] = []).push(idx);
          }
        } else {
          (index[value] != null ? index[value] : index[value] = []).push(idx);
        }
      }
    }
    return null;
  ...
  this.aggregate = function(me, points_or_entries, reducers) {
    var R, averages, cache, count, entries, entry, exclude, functions, i, ids_and_values, j, key, len, len1, points, points_or_entry, reducer, reducer_fallback, reducer_include, ref, ref1, ref2, ref3, ref4, ref5, sum, tag_keys, target, value;
    if (reducers == null) {
      reducers = {};
    }
    ...
    if (points.length > 0) {
      append(entries, this.match(me, points));
    }
    ...
    for (j = 0, len1 = entries.length; j < len1; j++) {
      ...
      for (key in entry) {
        ...
        reducer = (ref3 = reducers[key]) != null ? ref3 : reducer_fallback;
        switch (reducer) {
          case 'skip':
            continue;
          case 'list':
    ...
    /* averages */
    for (key in averages) {
      ref5 = averages[key], sum = ref5[0], count = ref5[1];
      R[key] = sum / count;
    }
  ...
  deepcopy_entry = function(me, entry) {
    return JSON.parse(JSON.stringify(entry));
  };

  deepcopy_reducers = function(me) {
    var R, key, reducers, value;
    if ((reducers = me['reducers']) == null) {
      return null;
    }
