(function() {
  //###########################################################################################################
  var CND, alert, append, as_number, as_numbers, badge, debug, echo, fuse, help, info, log, meld, mix, normalize_points, normalize_tag, reduce_tag, rpr, setting_keys_of_cover_and_intersect, sort_entries_by_insertion_order, sort_ids_by_insertion_order, unique, urge, warn, whisper, σ_minus_א, σ_misfit, σ_plus_א;
  ...
  // SERIALIZATION
  //-----------------------------------------------------------------------------------------------------------
  this.to_xjson = function(me) {
    var R, _, entry, key;
    R = {
      'index-keys': (function() {
        var results;
        results = [];
        for (key in me['indexes']) {
          results.push(key);
        }
        return results;
      })(),
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.new_from_xjson = function(xjson) {
    var R, description, entry, i, j, key, len, len1, ref, ref1;
    description = CND.XJSON.parse(xjson);
    R = this.new();
    ref = description['index-keys'];
    for (i = 0, len = ref.length; i < len; i++) {
      key = ref[i];
      this.add_index(R, key);
    }
    ref1 = description['entries'];
  ...
  this._match_or_intersect = function(me, mode, points, settings) {
    ...
    if (!CND.is_subset((keys = Object.keys(settings)), setting_keys_of_cover_and_intersect)) {
  ...
  this.aggregate.use = (me, reducers, settings = {}) => {
    ...
    if (!CND.is_subset((keys = Object.keys(settings)), ['memoize'])) {
    ...
    mix_entries_of_point = (point) => {
      ...
      point_count = (CND.isa_list(point)) ? point.length : 1;
      if (point_count !== 1) {
  ...
  as_number = function(x) {
    var length, type;
    if ((x === (-2e308) || x === (+2e308)) || CND.isa_number(x)) {
      return x;
    }
