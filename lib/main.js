...
(function() {
  var CND, alert, append, as_number, as_numbers, badge, debug, echo, help, info, log, rpr, unique, urge, warn, whisper,
    slice = [].slice,
  ...
  this.aggregate = function(me, points_or_entries, reducers) {
    var R, averages, cache, common, count, entries, entry, exclude, i, ids_and_values, j, key, len, len1, points, points_or_entry, reducer, reducer_fallback, ref, ref1, ref2, ref3, ref4, sum, tags_keys, target, value, values;
    if (reducers == null) {
      reducers = {};
    }
    ...
    for (key in R) {
      value = R[key];
      if (indexOf.call(tags_keys, key) < 0) {
        continue;
      }
      unique(value);
    }
