...
(function() {
  var CND, ISL, alert, badge, debug, demo_unassigned_unicode_codepoints, echo, find_ids_text, find_names_text, help, hex, include, info, list, log, rpr, s, show, test, urge, warn, whisper,
  ...
  if (module.parent == null) {
    include = ["tag 2a", "tag 2b", "complements"];
    this._prune();
    this._main();
    demo_unassigned_unicode_codepoints = function() {
      var chr, cid, cp_interval, cp_intervals, hi, i, j, len, lo, tags, u, ucps;
      console.time('A');
      ...
      console.timeEnd('B');
      for (cid = j = 885; j <= 915; cid = ++j) {
        chr = String.fromCodePoint(cid);
        tags = (ISL.aggregate(u, cid))['tag'].join(' ');
        debug(hex(cid), chr, CND.truth(indexOf.call(ucps.codepoints, cid) >= 0), tags);
      }
      return help(ISL.aggregate(u, 'a'));
    };
