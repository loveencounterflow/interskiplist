...
(function() {
  var CND, ISL, alert, badge, d, debug, echo, find_ids_text, find_names_text, help, hex, include, info, isl, log, rpr, show, test, urge, warn, whisper,
  ...
  find_names_text = function() {
    var R, me, points;
    me = arguments[0], points = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    R = ISL.names_of(me, ISL.find_ids_with_all_points.apply(ISL, [me].concat(slice.call(points))));
    R.sort();
    return R.join(',');
  };
  ...
  this["test interval tree 3"] = function(T) {
    var error_count, hi, i, id, intervals, isl, len, lo, ref;
    isl = ISL["new"]();
    ...
    f = function() {
      return this.add_range = (function(_this) {
        return function(me, name, lo, hi, value) {
          ...
          return _this.insert(me, lo, hi, interval_id, value);
        };
      })(this);
    };
    f.apply(ISL);
  ...
  if (module.parent == null) {
    include = ["test interval tree 1", "test interval tree 2", "test interval tree 3"];
    this._prune();
    this._main();
    isl = ISL["new"]();
    d = isl['%self'];
