...
(function() {
  ...
  this["readme example 2"] = function(T) {
    ...
    debug(ISL.find_names_with_all_points(samples, 'A'));
    debug(ISL.find_names_with_all_points(samples, '&'));
    debug(ISL.find_names_with_all_points(samples, '人'));
    debug(ISL.aggregate(samples, 'A'));
    debug(ISL.aggregate(samples, '&'));
    return debug(ISL.aggregate(samples, '人'));
  };
  ...
  if (module.parent == null) {
    include = ["test interval tree 1", "test interval tree 2", "test interval tree 3", "aggregation", "characters as points 1", "characters as points 2", "characters as points 3", "intervals_from_points", "new API for points", "readme example 1", "readme example 2", "intervals without ID, name"];
    this._prune();
    this._main();
  }
