...
(function() {
  ...
  this["readme example 2"] = function(T) {
    ...
    debug('rx2-1', ISL.find_names_with_all_points(samples, 'A'));
    debug('rx2-2', ISL.find_names_with_all_points(samples, '&'));
    debug('rx2-3', ISL.find_names_with_all_points(samples, '人'));
    debug('rx2-7', ISL.aggregate(samples, 'A'));
    debug('rx2-8', ISL.aggregate(samples, '&'));
    debug('rx2-9', ISL.aggregate(samples, '人'));
    replacers = {
      '*': 'list',
      name: 'include'
    };
    ...
    f = function(entries) {
      ...
      for (i = 0, len = entries.length; i < len; i++) {
        entry = entries[i];
        results.push(entry['font_family']);
      }
      return results;
    };
